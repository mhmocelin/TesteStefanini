...
const API_RESOURCE_URL_V2 = '/v2/persons';

export const getPersonsV2 = async (): Promise<PersonV2[]> => {
  try {
    const response = await apiClient.get<{ success: boolean; data: PersonV2[] }>(API_RESOURCE_URL_V2);
...
export const getPersonByIdV2 = async (id: string): Promise<PersonV2 | null> => {
  try {
    const response = await apiClient.get<{ success: boolean; data: PersonV2 }>(`${API_RESOURCE_URL_V2}/${id}`);
...
export const createPersonV2 = async (data: CreatePersonV2): Promise<PersonV2 | null> => {
  try {
    const response = await apiClient.post<{ success: boolean; data: PersonV2 }>(API_RESOURCE_URL_V2, data);
    return response.data.data;
  } catch (error) {
...
export const updatePersonV2 = async (id: string, data: UpdatePersonV2): Promise<PersonV2 | null> => {
  try {
    const response = await apiClient.put<{ success: boolean; data: PersonV2 }>(`${API_RESOURCE_URL_V2}/${id}`, data);
    return response.data.data;
  } catch (error) {
...
export const deletePersonV2 = async (id: string): Promise<boolean> => {
  try {
    await apiClient.delete(`${API_RESOURCE_URL_V2}/${id}`);
    return true;
  } catch (error) {
